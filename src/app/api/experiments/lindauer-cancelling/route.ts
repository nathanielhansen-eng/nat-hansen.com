...
const CONDITIONS = new Set<string>(["helping", "harming", "cancelling"]);
...
function validate(body: unknown): Submission | null {
  ...
  if (typeof b.condition !== "string" || !CONDITIONS.has(b.condition)) return null;
...
export async function POST(request: Request) {
  ...
  const key = `lindauer-cancelling/${submission.session}/${id}.json`;
