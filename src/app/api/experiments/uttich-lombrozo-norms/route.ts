...
interface Submission {
  ...
  submittedAt: string;
  durationMs: number;
  /** Single-design study (Experiment 1, CEO version); kept for launcher-URL parity. */
  study: 1 | 2;
  /** Norm TYPE cell: a moral (environmental) norm or a purely conventional colour norm. */
  normType: NormType;
  /** Norm STATUS cell: the outcome conforms to or violates the stipulated norm. */
  normStatus: NormStatus;
  /** 1–7 appropriateness rating for calling the side effect "intentional". */
  rating: number;
  ratingRtMs: number;
...
function validate(body: unknown): Submission | null {
  ...
  if (b.normType !== "moral" && b.normType !== "conventional") return null;
  if (b.normStatus !== "conforming" && b.normStatus !== "violating") return null;
...
export async function POST(request: Request) {
  ...
  const key = `uttich-lombrozo-norms/${submission.session}/${id}.json`;
